    ...
    public JmmParserResult parse(String jmmCode, String startingRule, Map<String, String> config) {
        ...
        try {
            ...
            return AntlrParser.parse(lex, parser, startingRule)
                    ...
                    .orElseGet(() -> JmmParserResult.newError(new Report(ReportType.ERROR, Stage.SYNTATIC, parser.getCurrentToken().getLine(), parser.getCurrentToken().getCharPositionInLine(),
                            "There were " + parser.getNumberOfSyntaxErrors() + " syntax error(s) during parsing, terminating")));
